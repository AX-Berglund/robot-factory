...
{
	private static final Logger LOGGER = Logger.getLogger(KolmogorovWeightedPerfectMatching.class.getName());
    ...
    private void printState()
    {
        ...
        for (int i = 0; i < state.nodeNum; i++) {
            ...
            if (node.matched != null) {
                ...
                if (matchedEdge.head[0].matched == null || matchedEdge.head[1].matched == null) {
                    LOGGER.fine("Problem with edge " + matchedEdge);
        ...
        for (int i = 0; i < state.edgeNum; i++) {
            LOGGER.fine(edges[i] + (matched.contains(edges[i]) ? ", matched" : ""));
        }
    }
    ...
    {
        LOGGER.fine(state.nodeNum + " " + state.edgeNum);
        for (int i = 0; i < state.nodeNum; i++) {
            LOGGER.fine(state.graphVertices.get(i) + " -> " + state.nodes[i]);
        }
    }
