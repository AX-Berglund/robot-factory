...
{	
	private static final Logger LOGGER = Logger.getLogger(BlossomVDualUpdater.class.getName());
    ...
    public double updateDuals(BlossomVOptions.DualUpdateStrategy type)
    {
        ...
        if (KolmogorovWeightedPerfectMatching.DEBUG) {
            LOGGER.fine("Start updating duals");
        }
    ...
    public boolean updateDualsSingle(BlossomVTree tree)
    {
        ...
        if (eps > tree.eps) {
            ...
            tree.eps = eps;
            if (KolmogorovWeightedPerfectMatching.DEBUG) {
                LOGGER.fine("Updating duals: now eps of " + tree + " is " + eps);
            }
        }
    ...
    {
        if (KolmogorovWeightedPerfectMatching.DEBUG) {
            LOGGER.fine("Multiple tree fixed delta approach");
        }
        BlossomVEdge augmentEdge = null;
        double eps = INFINITY;
        double augmentEps = INFINITY;
